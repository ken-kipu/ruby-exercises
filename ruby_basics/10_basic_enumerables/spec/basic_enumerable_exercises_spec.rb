...
RSpec.describe 'Basic Enumerable Exercises' do
  ...
  describe 'display number order exercise' do
    
    it 'outputs each number of strings in order' do
      expect($stdout).to receive(:puts).with("1! ")
      expect($stdout).to receive(:puts).with("2! ")
      expect($stdout).to receive(:puts).with("3! ")
      numbers = ['cookies', 'cake', 'ice cream']
      display_number_order(numbers)
    end

    it 'outputs each number of integers in order' do
      expect($stdout).to receive(:puts).with("number #1 is 553")
      expect($stdout).to receive(:puts).with("number #2 is 554")
      expect($stdout).to receive(:puts).with("number #3 is 555")
      numbers = [553, 554, 555]
      display_number_order(numbers)
    end
  end
